	...
	if args.data == "big":
		model.fit_generator(
			...
			callbacks=[EarlyStopping(patience=patience),
					   ModelCheckpoint("weights_big/weights.{epoch:03d}-{val_loss:.5f}.hdf5", monitor='val_loss', verbose=1, save_best_only=True, mode='auto', period=0.01), history, My_Callback()
					   ]
		)
